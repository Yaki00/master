...
function runMeta(parsed: Record<string, unknown>): Record<string, unknown> | null {
  ...
  return asRecord(nested?.meta) ?? asRecord(parsed.meta);
}
...
export function looksLikeOpenclawFailure(raw: string | null | undefined): boolean {
  ...
  if (rec) {
    const status = String(rec.status ?? rec.summary ?? "").toLowerCase();
...
export function interpretOpenclawOutput(raw: string | null | undefined): { ok: boolean; text: string } {
  const blob = raw ?? "";
  if (looksLikeOpenclawFailure(blob)) {
    return { ok: false, text: humanizeOpenclawFailure(blob) };
  }
  return { ok: true, text: extractReplyText(blob) };
}
